...
import { isMaster } from '#/utils/isMaster.ts';
import { logger } from '#/utils/useLogger.ts';
const dirPath = path.resolve(import.meta.dirname, '../routes');
console.log('dirPath', dirPath);
const router = Router();
const routes: { url: string; method: string }[] = [];

const getRouteSegmentPriority = (name: string) => {
  if (/^index(\.|$)/.test(name) || name === 'index') {
    return 1;
  }
  if (/^\[.*\]/.test(name)) {
    return 2;
...
const sortRouteEntries = (files: fs.Dirent[]) => {
  return [...files].sort((a, b) => {
    if (a.isDirectory() !== b.isDirectory()) {
      return a.isDirectory() ? -1 : 1;
    }

    const priorityDiff = getRouteSegmentPriority(a.name) - getRouteSegmentPriority(b.name);
    if (priorityDiff !== 0) {
      return priorityDiff;
    }
...
export const useRouters = async (app: express.Application) => {
  try {
    async function getAllFiles(root: string) {
      const result: string[] = [];
      const files = sortRouteEntries(
        fs
          .readdirSync(root, { withFileTypes: true })
          .filter(
            (file) => file.name[0] !== '.' && !/.map$/.test(file.name) && !/^type/.test(file.name),
          ),
      );
      for (const file of files) {
        ...
        if (file.isDirectory()) {
          ...
          const asyncRoute = await import(fileUrl.href);
          ...
          try {
            ...
            let realPath = `${dirname}/${routePath}`
              .replace(/\[.*\]/, (str) => {
                if (str.length === 2) {
                  return '';
                }
              ...
              .replace(/\/+/g, '/');
            if (realPath !== '/') {
              realPath = realPath.endsWith('/') ? realPath.slice(0, -1) : realPath;
            }
            ...
            logger.error('router注册失败: ' + error.message);
          }
        ...
        if (isMaster) {
          ...
          for (const route of routes) {
            ...
            const some = result.rows.some(
              (item: any) => item.url === route.url && item.method === route.method,
            );
            // 不存在 ===> 创建
            if (!some) {
