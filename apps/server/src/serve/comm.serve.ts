...
export abstract class CommServe<T = any> {
  ...
  constructor(Table: ReturnType<typeof db.define>) {
    ...
    this.Table.create = (async (data: Record<any, any>, options?: Parameters<typeof create>[1]) => {
      data.update_time = db.fn('NOW');
      const result = await create.call(this.Table, data, options);
      return result;
    }) as any;
    this.attributes = Object.keys(this.Table.getAttributes());
  }
  public async clear() {
   ...
   * @returns  删除的条数
   */
  public async deleteById(id: number | number[] | string | string[] | (string | number)[]) {
   ...
   */
  public filterWhereAttributes(where: Record<any, any>, filterType: any[] = [undefined, null, '']) {
