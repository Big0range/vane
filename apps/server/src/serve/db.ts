...
import { logger, useMysqlLogger } from '#/utils/useLogger.ts';
...
const l = [mysqlHosts.length, mysqlUsers.length, mysqlPasswords.length, mysqlPorts.length];
...
// 心跳检测
// setInterval(() => {
//   sequelize.query('SELECT 1').catch((error) => logger.error('数据库心跳失败', error));
// }, 10000);
