...
import { sysMenuRoutesServe } from './menu_routes.serve.ts';
...
class SysMenuServe extends CommServe<TMenu> {
  ...
  public async update(data: TMenu & { routes?: number[] }) {
    if (!data.id) {
    ...
    await sysMenuRoutesServe.deleteByMenuId(data.id);
    if (data.routes && data.routes.length > 0) {
      await sysMenuRoutesServe.bulkCreate(
  ...
  public async create(data: Omit<TMenu, 'id'> & { routes?: number[] }) {
    if (!data.parent_id) {
    ...
    const result = await this.Table.create(this.filterWhereAttributes(data));
    if (data.routes && data.routes.length > 0) {
      await sysMenuRoutesServe.bulkCreate(
        data.routes.map((item) => ({
          menu_id: result.dataValues.id,
          route_id: item,
        })),
      );
    }
  ...
  public async findAllMenu() {
    ...
    menus.forEach((item) => {
      ...
      for (let i = 0; i < menus.length; i++) {
        ...
        const id = element.id;
        if (parent_id === id) {
          element.children ? null : (element.children = []);
