import sequelize, { DataTypes, type FindOptions } from 'sequelize';
...
export class SysRoutesServer extends CommServe<TRoutes> {
  ...
  public async getRouteList(
    ...
    result.rows = result.rows.map((item) => {
      const route = item;
      (route as any).sysWhiteApi = routeWhiteObj[route.url + '|' + route.method] || false;
      if ((route as any).sysWhiteApi) {
        route.auth = '0';
      }
      return route;
    });
  ...
  public async getRouteWhitelist(): Promise<any[]> {
    ...
    if (redisResult && redisResult !== '[]') {
      return [...JSON.parse(redisResult), ...SysRoutesServer.constantRouteWhiteList];
    }
    ...
    const list = [
      ...result.map((itme: any) => itme.dataValues),
