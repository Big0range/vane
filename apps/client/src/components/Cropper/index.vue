        ...
        <div class="dialog-footer">
          <div class="left">
            <n-button-group>
              ...
              <n-button round @click="rotateRight">
                <template #icon>
                  <n-icon>
                    <ReloadOutline />
                  </n-icon>
          ...
          <div>
            <n-button @click="GetData">裁剪</n-button>
          </div>
        </div>
      </template>
    </n-modal>
...
import { NIcon, type ImageInst } from 'naive-ui';
const imageRef = ref<ImageInst[]>([]);
const to = ref<string | HTMLElement>('');
onMounted(() => {
  to.value = document.getElementById('cropper-modal')!;
});
const images = ref<
  {
    key: number;
...
const preview = (i: number) => {
  console.log('aaaaaaaaaaaaa', imageRef.value);
  imageRef.value[i].showPreview();
};

const remove = (i: number) => {
  const values = [...props.modelValue];
  values.splice(i, 1);
  console.log("'update:modelValue', values", values);
