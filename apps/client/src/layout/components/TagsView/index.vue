<template>
  <div class="tags-view__container">
    <scroll-pane ref="scrollPaneRef" class="tags-view__wrapper">
      <router-link
        v-for="tag in visitedViews"
        :key="tag.path"
        :data-path="tag.path"
        :data-style="setting.tagsStyle"
        :class="isActive(tag) ? 'active' : ''"
        :data-type="tag.meta.type"
        :to="{ path: tag.path, query: tag.query }"
        class="tags-view__item"
        @click.middle="
          !isAffix(tag) ? closeSelectedTag(tag) : closeSelectedTag(tag)
        "
      ...
      >
        <div class="flex items-center">
          ...
          <span
            v-if="!isAffix(tag)"
            class="inline-flex items-center ml-6 icon-close"
...
const { tagsView, permission, setting } = useStore();
...
function isFirstView() {
  try {
    ...
    console.error('判断是否是第一个标签失败:', err);
    return false;
  }
}
...
function isLastView() {
  try {
    ...
    console.error('判断是否是最后一个标签失败:', err);
    return false;
  }
}
...
.tags-view__container {
  ...
  border-bottom: 1px solid var(--app-tags-view-bg-color, #d8dce5);
  box-shadow:
  ...
  .tags-view__wrapper {
    ...
    .tags-view__item[data-style='default'] {
      height: 38px;
      line-height: 38px;
      ...
      &:hover {
        ...
        mask: url('@/assets/layout/tags-mask.png');
