...
export const useCollectorLogger = (name: number | string) => {
  // fs.unlinkSync(path.join(process.cwd(), `logs/collector/${name}.log`));
  const collectorLogger = winston.createLogger({
    level: 'info',
    format: combine(
      timestamp({ format: 'YYYY-MM-DD HH:mm:ss' }), // 添加时间戳
      logFormat, // 应用自定义格式
    ...
    transports: [
      ...
      new winston.transports.File({ filename: `logs/collector/${name}.log` }),
    ],
  });

  collectorLogger.error = function (message: string, ...args: any[]) {
    collectorLogger.info(message);
    collectorLogger.error(message, ...args);
  } as any;
